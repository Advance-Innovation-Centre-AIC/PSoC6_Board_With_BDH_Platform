...
QueueHandle_t xQueueCapsenseBt1 = NULL;
QueueHandle_t xQueueCapsenseBt2 = NULL;
QueueHandle_t xQueueCapsenseSlider = NULL;
...
void capsense_task(void* param)
{
    ...
    xQueueCapsenseBt1 = xQueueCreate( 1, sizeof(uint8_t));
    xQueueCapsenseBt2 = xQueueCreate( 1, sizeof(uint8_t));
    xQueueCapsenseSlider = xQueueCreate( 10, sizeof(uint16_t));
...
//        printf("Button 0 pressed\n");
    	...
    	if (xQueueCapsenseBt1 != NULL) {
			if (!xQueueSend(xQueueCapsenseBt1, &button0_value, pdMS_TO_TICKS(50))) {
			 //	puts("Failed to send item to queue within 50ms");
		 }
		}
    }else
    {
...
//        printf("Button 1 pressed\n");
		...
		if (xQueueCapsenseBt2 != NULL) {
			if (!xQueueSend(xQueueCapsenseBt2, &button1_value, pdMS_TO_TICKS(50))) {
			 //	puts("Failed to send item to queue within 50ms");
		 }
		}
    }else
...
//        printf("Slider position %d\n",slider_pos);
		if (xQueueCapsenseSlider != NULL) {
			if (!xQueueSend(xQueueCapsenseSlider, &slider_pos, pdMS_TO_TICKS(50))) {
			 //	puts("Failed to send item to queue within 50ms");
		 }
		}
    }
