 ...
 *
 ******************************************************************************/
void biil_pub_task(void *pvParameters)
{
	...
	char data_buf2[PAYLOAD_MQTT_SIZE];
...
//    publisher_data_t publisher_q_data;
   ...
   while (true)
   {
	  ...
	  if (xQueueData1MQTT != NULL) {
		  if (xQueueReceive(xQueueData1MQTT, &data_packet1, pdMS_TO_TICKS(0)) == pdPASS) {
			  // printf("Received capsense_bt1: %d\n", capsense_bt1);
			  //
			  print_packet1_json(&data_packet1, data_buf);

			  publish_info.topic = MQTT_PUB_CY8CKIT_62S2_40312_TOPIC;
			  publish_info.topic_len = (sizeof(MQTT_PUB_CY8CKIT_62S2_40312_TOPIC) - 1);

			  publish_info.payload = (const char* )&data_buf[0];
			  publish_info.payload_len = strlen(data_buf);

			   printf("  Publisher: Publishing '%s' on the topic '%s' \r\n",
			   (char *) &data_buf, publish_info.topic);

//			   printf("/*%s*/",(char *) &data_buf);
...
//			  print_heap_usage("publisher_task: After publishing an MQTT message");
	  ...
	  if (xQueueData2MQTT != NULL) {
		  if (xQueueReceive(xQueueData2MQTT, &data_packet2, pdMS_TO_TICKS(0)) == pdPASS) {
			  // printf("Received capsense_bt1: %d\n", capsense_bt1);
			  //
			  print_packet2_json(&data_packet2, data_buf2);

			  publish_info.topic = MQTT_PUB_CY8CKIT_028_SENSE_TOPIC;
			  publish_info.topic_len = (sizeof(MQTT_PUB_CY8CKIT_028_SENSE_TOPIC) - 1);

			  publish_info.payload = (const char* )&data_buf2[0];
			  publish_info.payload_len = strlen(data_buf2);

			   printf("  Publisher: Publishing '%s' on the topic '%s' \r\n",
			   (char *) &data_buf2, publish_info.topic);

//			   printf("/*%s*/",(char *) &data_buf);


			  result2 = cy_mqtt_publish(mqtt_connection, &publish_info);
